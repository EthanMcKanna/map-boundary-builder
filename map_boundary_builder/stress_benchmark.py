...
REAL_SCREENSHOT_HARD_GATE_PRESET_VERSION = 13
FOCUSED_REAL_SCREENSHOT_GATE_PRESET_NAME = "focused-real-screenshot-gate"
FOCUSED_REAL_SCREENSHOT_GATE_PRESET_VERSION = 11
...
REAL_SCREENSHOT_HARD_GATE_BASELINE_REPEAT_HIDDEN_OVERLAP_TOTAL_REGRESSION_S = 0.10
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--max-baseline-repeat-ocr-total-max-regression-s",
        type=float,
        default=None,
        help=(
            "Exit non-zero when aggregate or per-case repeat-profile OCR engine "
            "total or substage max duration is more than this many seconds slower "
            "than --compare-baseline-report."
        ),
    )
    parser.add_argument(
        "--max-baseline-repeat-ocr-count-p95-regression",
        type=float,
        default=None,
        help=(
            "Exit non-zero when aggregate or per-case repeat-profile OCR engine "
            "count p95 increases by more than this many boxes or labels versus "
            "--compare-baseline-report."
        ),
    )
    parser.add_argument(
        ...
        help=(
            "Exit non-zero when aggregate or per-case repeat-profile OCR engine "
            "count max increases by more than this many boxes or labels versus "
            "--compare-baseline-report."
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "Exit non-zero when aggregate repeat-profile hidden OCR overlap total_s "
            "increases by more than this many seconds versus --compare-baseline-report."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        metavar="METRIC=SECONDS",
        help=(
            "Fail when a profiled repeat OCR engine metric max exceeds this budget. "
            "Repeat or comma-separate entries such as det_elapsed_s=0.3,rec_elapsed_s=0.6,total_s=0.9."
        ),
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    if (
        ...
        parser.error("--max-baseline-repeat-stage-p95-regression-s requires --compare-baseline-report")
    ...
    if (
        args.max_baseline_repeat_ocr_total_max_regression_s is not None
        and not args.compare_baseline_report
    ):
        parser.error("--max-baseline-repeat-ocr-total-max-regression-s requires --compare-baseline-report")
    if (
        args.max_baseline_repeat_ocr_count_p95_regression is not None
        and not args.compare_baseline_report
    ):
        parser.error("--max-baseline-repeat-ocr-count-p95-regression requires --compare-baseline-report")
    if (
        args.max_baseline_repeat_ocr_count_max_regression is not None
        and not args.compare_baseline_report
    ):
        parser.error("--max-baseline-repeat-ocr-count-max-regression requires --compare-baseline-report")
    if (
        args.max_baseline_repeat_ocr_overlap_hidden_total_regression_s is not None
        and not args.compare_baseline_report
    ):
        parser.error(
            "--max-baseline-repeat-ocr-overlap-hidden-total-regression-s "
            "requires --compare-baseline-report"
        )
    ...
    if (
        ...
        parser.error("--max-baseline-repeat-stage-p95-regression-s must be non-negative")
    if (
    ...
    if (
        args.max_baseline_ocr_count_regression is not None
    ...
    if (
        ...
        parser.error("--max-baseline-stage-total-regression-s must be non-negative")
    if (
        args.max_baseline_repeat_ocr_total_p95_regression_s is not None
    ...
    if (
        args.max_baseline_repeat_ocr_total_max_regression_s is not None
        and args.max_baseline_repeat_ocr_total_max_regression_s < 0.0
    ):
        parser.error("--max-baseline-repeat-ocr-total-max-regression-s must be non-negative")
    if (
        args.max_baseline_repeat_ocr_count_p95_regression is not None
        and args.max_baseline_repeat_ocr_count_p95_regression < 0.0
    ):
        parser.error("--max-baseline-repeat-ocr-count-p95-regression must be non-negative")
    if (
        args.max_baseline_repeat_ocr_count_max_regression is not None
        and args.max_baseline_repeat_ocr_count_max_regression < 0.0
    ):
        parser.error("--max-baseline-repeat-ocr-count-max-regression must be non-negative")
    if (
        args.max_baseline_repeat_ocr_overlap_hidden_total_regression_s is not None
        and args.max_baseline_repeat_ocr_overlap_hidden_total_regression_s < 0.0
    ):
        parser.error(
            "--max-baseline-repeat-ocr-overlap-hidden-total-regression-s "
    ...
    try:
        ...
        parser.error(str(exc))
    try:
        max_repeat_ocr_engine_max_duration_s = parse_metric_duration_budgets(
            args.max_repeat_ocr_engine_max_duration_s
        )
    except ValueError as exc:
        parser.error(str(exc))
    try:
    ...
    report = run_stress_benchmark(
        ...
        max_repeat_ocr_engine_max_duration_s=max_repeat_ocr_engine_max_duration_s,
        ...
        max_baseline_repeat_ocr_total_max_regression_s=(
            args.max_baseline_repeat_ocr_total_max_regression_s
        ),
        max_baseline_repeat_ocr_count_p95_regression=(
            args.max_baseline_repeat_ocr_count_p95_regression
        ),
        max_baseline_repeat_ocr_count_max_regression=(
            args.max_baseline_repeat_ocr_count_max_regression
        ),
        max_baseline_repeat_ocr_overlap_hidden_total_regression_s=(
            args.max_baseline_repeat_ocr_overlap_hidden_total_regression_s
        ),
...
def apply_real_screenshot_hard_gate_preset(args: argparse.Namespace, parser: argparse.ArgumentParser) -> None:
    ...
    if args.compare_baseline_report:
        ...
        if args.max_baseline_repeat_ocr_total_max_regression_s is None:
            args.max_baseline_repeat_ocr_total_max_regression_s = (
                REAL_SCREENSHOT_HARD_GATE_BASELINE_REGRESSION_S
            )
        if args.max_baseline_repeat_ocr_count_p95_regression is None:
            args.max_baseline_repeat_ocr_count_p95_regression = (
                REAL_SCREENSHOT_HARD_GATE_BASELINE_COUNT_REGRESSION
            )
        if args.max_baseline_repeat_ocr_count_max_regression is None:
            args.max_baseline_repeat_ocr_count_max_regression = (
                REAL_SCREENSHOT_HARD_GATE_BASELINE_COUNT_REGRESSION
            )
        if args.max_baseline_repeat_ocr_overlap_hidden_total_regression_s is None:
            args.max_baseline_repeat_ocr_overlap_hidden_total_regression_s = (
                REAL_SCREENSHOT_HARD_GATE_BASELINE_REPEAT_HIDDEN_OVERLAP_TOTAL_REGRESSION_S
            )
    if args.repeat_profile_runs == 0:
    ...
    args.max_repeat_ocr_engine_max_duration_s = prepend_default_budget_entries(
        ["total_s=0.7"],
        args.max_repeat_ocr_engine_max_duration_s,
    )
    ...
    if args.min_ocr_call_contract_rows is None:
        args.min_ocr_call_contract_rows = 50
    if args.min_ocr_count_contract_rows is None:
        args.min_ocr_count_contract_rows = 45
...
def run_stress_benchmark(
    ...
    *,
    ...
    max_repeat_ocr_engine_max_duration_s: dict[str, float] | None = None,
    ...
    max_baseline_repeat_ocr_count_p95_regression: float | None = None,
    max_baseline_repeat_ocr_count_max_regression: float | None = None,
    max_baseline_repeat_ocr_overlap_hidden_total_regression_s: float | None = None,
    ...
    if (
        ...
        raise ValueError("max_baseline_repeat_stage_p95_regression_s must be non-negative")
    ...
    if (
        max_baseline_ocr_count_regression is not None
    ...
    if (
        ...
        raise ValueError("max_baseline_stage_total_regression_s must be non-negative")
    if (
    ...
    if (
        max_baseline_repeat_ocr_total_max_regression_s is not None
        and max_baseline_repeat_ocr_total_max_regression_s < 0.0
    ):
        raise ValueError("max_baseline_repeat_ocr_total_max_regression_s must be non-negative")
    if (
        max_baseline_repeat_ocr_count_p95_regression is not None
        and max_baseline_repeat_ocr_count_p95_regression < 0.0
    ):
        raise ValueError("max_baseline_repeat_ocr_count_p95_regression must be non-negative")
    if (
        max_baseline_repeat_ocr_count_max_regression is not None
        and max_baseline_repeat_ocr_count_max_regression < 0.0
    ):
        raise ValueError("max_baseline_repeat_ocr_count_max_regression must be non-negative")
    if (
        max_baseline_repeat_ocr_overlap_hidden_total_regression_s is not None
        and max_baseline_repeat_ocr_overlap_hidden_total_regression_s < 0.0
    ):
        raise ValueError(
            "max_baseline_repeat_ocr_overlap_hidden_total_regression_s must be non-negative"
        )
    ...
    ocr_engine_max_budgets = dict(max_repeat_ocr_engine_max_duration_s or {})
    ...
    if (
        ...
        report["latency_budget"] = build_latency_budget_summary(
            ...
            max_repeat_ocr_engine_max_duration_s=ocr_engine_max_budgets,
    ...
    if compare_baseline_report is not None:
        report["baseline_comparison"] = compare_stress_reports(
            ...
            max_repeat_ocr_engine_total_max_regression_s=(
                max_baseline_repeat_ocr_total_max_regression_s
            ),
            max_repeat_ocr_engine_count_p95_regression=(
                max_baseline_repeat_ocr_count_p95_regression
            ),
            max_repeat_ocr_engine_count_max_regression=(
                max_baseline_repeat_ocr_count_max_regression
            ),
            max_repeat_ocr_overlap_hidden_total_regression_s=(
                max_baseline_repeat_ocr_overlap_hidden_total_regression_s
            ),
        )
...
def compare_stress_reports(
    ...
    *,
    ...
    max_repeat_ocr_engine_count_p95_regression: float | None = None,
    max_repeat_ocr_engine_count_max_regression: float | None = None,
    max_repeat_ocr_overlap_hidden_total_regression_s: float | None = None,
) -> dict[str, Any]:
    ...
    if (
        ...
        or max_repeat_ocr_engine_count_p95_regression is not None
        or max_repeat_ocr_engine_count_max_regression is not None
        or max_repeat_ocr_overlap_hidden_total_regression_s is not None
    ):
...
def baseline_regression_budget(
    ...
    *,
    ...
    if max_ocr_engine_count_regression is not None:
        max_ocr_count = round(float(max_ocr_engine_count_regression), 6)
        budget["max_ocr_engine_count_regression"] = max_ocr_count
        if isinstance(latency_deltas, list):
            for delta in latency_deltas:
                if not isinstance(delta, dict):
                    continue
                count_deltas = delta.get("ocr_engine_count_delta")
                if not isinstance(count_deltas, dict):
                    ...
                    violation = {
                        ...
                        "slug": delta.get("slug"),
                        "max_ocr_engine_count_regression": max_ocr_count,
                    }
                    if baseline_calls is not None:
                        violation["baseline_ocr_engine_calls"] = round(baseline_calls, 6)
                    if candidate_calls is not None:
                    ...
                    violations.append(violation)
                    continue
                baseline_count_values = delta.get("baseline_ocr_engine_count")
                candidate_count_values = delta.get("candidate_ocr_engine_count")
                for metric in OCR_ENGINE_COUNT_DISPLAY_KEYS:
                    count_delta = parse_signed_float(count_deltas.get(metric))
                    if count_delta is None or count_delta <= max_ocr_engine_count_regression:
                        continue
                    violation = {
                        "kind": "primary_ocr_count_regression_exceeded",
                        "slug": delta.get("slug"),
                        "metric": metric,
                        "delta_count": round(count_delta, 6),
                        "max_ocr_engine_count_regression": max_ocr_count,
                    }
                    if isinstance(baseline_count_values, dict):
                        baseline_count = parse_nonnegative_count_metric(
                            baseline_count_values.get(metric)
                        )
                        if baseline_count is not None:
                            violation["baseline_ocr_engine_count"] = round(baseline_count, 6)
                    if isinstance(candidate_count_values, dict):
                        candidate_count = parse_nonnegative_count_metric(
                            candidate_count_values.get(metric)
                        )
    ...
    if max_stage_total_regression_s is not None:
        ...
        stage_deltas = comparison.get("stage_duration_delta_s")
        if isinstance(stage_deltas, dict):
            for stage in sorted(
                ...
                delta = stage_deltas.get(stage)
                if not isinstance(delta, dict):
                    continue
                stage_delta = parse_signed_float(delta.get("delta_s"))
                ...
                baseline_stage_total = parse_nonnegative_float(delta.get("baseline"))
                candidate_stage_total = parse_nonnegative_float(delta.get("candidate"))
                if baseline_stage_total is not None:
                    violation["baseline_stage_total_s"] = round(baseline_stage_total, 6)
                if candidate_stage_total is not None:
                    violation["candidate_stage_total_s"] = round(candidate_stage_total, 6)
                violations.append(violation)
        else:
            violations.append(
                {
                    "kind": "primary_stage_total_delta_missing",
                    "max_stage_total_regression_s": max_stage_total,
                }
            )
        if isinstance(latency_deltas, list):
    ...
    if max_repeat_stage_p95_regression_s is not None:
        ...
        repeat_delta = comparison.get("repeat_profile_delta")
        stage_duration = (
            repeat_delta.get("stage_duration_s")
            if isinstance(repeat_delta, dict)
            else None
        )
        if isinstance(stage_duration, dict):
            for stage in sorted(
                ...
                p95_delta = repeat_delta_metric_value(
                    repeat_delta,
                ...
                if p95_delta is None or p95_delta <= max_repeat_stage_p95_regression_s:
                    continue
                violation = {
                    "kind": "repeat_stage_p95_regression_exceeded",
                    "stage": stage,
                    "delta_s": round(p95_delta, 6),
                ...
                if baseline_stage_p95 is not None:
                    violation["baseline_stage_p95_duration_s"] = round(baseline_stage_p95, 6)
                if candidate_stage_p95 is not None:
                    violation["candidate_stage_p95_duration_s"] = round(candidate_stage_p95, 6)
                violations.append(violation)
        else:
            violations.append(
                {
                    "kind": "repeat_stage_p95_delta_missing",
                    "max_repeat_stage_p95_regression_s": max_repeat_stage,
                }
            )
        case_deltas = comparison.get("repeat_profile_case_deltas")
        ...
        if isinstance(case_deltas, list):
            for case_delta in case_deltas:
                ...
                for stage in OCR_ENGINE_STAGE_MAX_KEYS:
                    ...
                    violations.append(violation)
    if max_repeat_ocr_engine_total_max_regression_s is not None:
        max_repeat_ocr_max = round(float(max_repeat_ocr_engine_total_max_regression_s), 6)
        budget["max_repeat_ocr_engine_total_max_regression_s"] = max_repeat_ocr_max
        repeat_delta = comparison.get("repeat_profile_delta")
        repeat_ocr_max_delta = (
            repeat_delta_metric_value(
                repeat_delta,
                ("ocr_engine_stage_duration_s", "total_s", "max_duration_s"),
                "delta_s",
            )
            if isinstance(repeat_delta, dict)
            else None
        )
        if repeat_ocr_max_delta is None:
            violations.append(
                {
                    "kind": "repeat_ocr_total_max_delta_missing",
                    "max_repeat_ocr_engine_total_max_regression_s": max_repeat_ocr_max,
                }
            )
        elif repeat_ocr_max_delta > max_repeat_ocr_engine_total_max_regression_s:
            repeat_ocr_max = repeat_delta_metric_value(
                repeat_delta,
                ("ocr_engine_stage_duration_s", "total_s", "max_duration_s"),
                "candidate",
            )
            baseline_ocr_max = repeat_delta_metric_value(
                repeat_delta,
                ("ocr_engine_stage_duration_s", "total_s", "max_duration_s"),
                "baseline",
            )
            violation = {
                "kind": "repeat_ocr_total_max_regression_exceeded",
                "delta_s": round(repeat_ocr_max_delta, 6),
                "max_repeat_ocr_engine_total_max_regression_s": max_repeat_ocr_max,
            }
            if baseline_ocr_max is not None:
                violation["baseline_ocr_engine_total_max_duration_s"] = round(baseline_ocr_max, 6)
            if repeat_ocr_max is not None:
                violation["candidate_ocr_engine_total_max_duration_s"] = round(repeat_ocr_max, 6)
            violations.append(violation)
        if isinstance(repeat_delta, dict):
            for stage in OCR_ENGINE_STAGE_MAX_KEYS:
                if stage == "total_s":
                    continue
                stage_max_delta = repeat_delta_metric_value(
                    repeat_delta,
                    ("ocr_engine_stage_duration_s", stage, "max_duration_s"),
                    "delta_s",
                )
                if (
                    stage_max_delta is None
                    or stage_max_delta <= max_repeat_ocr_engine_total_max_regression_s
                ):
                    continue
                violation = {
                    "kind": "repeat_ocr_stage_max_regression_exceeded",
                    "stage": stage,
                    "delta_s": round(stage_max_delta, 6),
                    "max_repeat_ocr_engine_total_max_regression_s": max_repeat_ocr_max,
                }
                baseline_stage_max = repeat_delta_metric_value(
                    repeat_delta,
                    ("ocr_engine_stage_duration_s", stage, "max_duration_s"),
                    "baseline",
                )
                candidate_stage_max = repeat_delta_metric_value(
                    repeat_delta,
                    ("ocr_engine_stage_duration_s", stage, "max_duration_s"),
                    "candidate",
                )
                if baseline_stage_max is not None:
                    violation["baseline_ocr_engine_stage_max_duration_s"] = round(
                        baseline_stage_max,
                        6,
                    )
                if candidate_stage_max is not None:
                    violation["candidate_ocr_engine_stage_max_duration_s"] = round(
                        candidate_stage_max,
                        6,
                    )
                violations.append(violation)
        ...
        if isinstance(case_deltas, list):
            for case_delta in case_deltas:
                if not isinstance(case_delta, dict):
                    continue
                max_delta = repeat_case_delta_ocr_stage_value(
                    case_delta,
                    "total_s",
                    "max_duration_s",
                    "delta_s",
                ...
                if (
                    ...
                    and max_delta > max_repeat_ocr_engine_total_max_regression_s
                ):
                    violation = {
                        "kind": "repeat_ocr_case_total_max_regression_exceeded",
                        "slug": case_delta.get("slug"),
                        "delta_s": round(max_delta, 6),
                        "max_repeat_ocr_engine_total_max_regression_s": max_repeat_ocr_max,
                    }
                    baseline_case_max = repeat_case_delta_ocr_stage_value(
                        case_delta,
                        "total_s",
                        "max_duration_s",
                        "baseline",
                    )
                    candidate_case_max = repeat_case_delta_ocr_stage_value(
                        case_delta,
                        "total_s",
                        "max_duration_s",
                        "candidate",
                    ...
                    if baseline_case_max is not None:
                        violation["baseline_ocr_engine_total_max_duration_s"] = round(
                            baseline_case_max,
                            6,
                        )
                    if candidate_case_max is not None:
                        violation["candidate_ocr_engine_total_max_duration_s"] = round(
                            candidate_case_max,
                            6,
                        )
                    violations.append(violation)
                ...
                for stage in OCR_ENGINE_STAGE_MAX_KEYS:
                    if stage == "total_s":
                        continue
                    stage_max_delta = repeat_case_delta_ocr_stage_value(
                        case_delta,
                        stage,
                        "max_duration_s",
                        "delta_s",
                    ...
                    if (
                        ...
                        continue
                    violation = {
                        "kind": "repeat_ocr_case_stage_max_regression_exceeded",
                        "slug": case_delta.get("slug"),
                        "stage": stage,
                        "delta_s": round(stage_max_delta, 6),
                        "max_repeat_ocr_engine_total_max_regression_s": max_repeat_ocr_max,
                    }
                    baseline_case_stage_max = repeat_case_delta_ocr_stage_value(
                        case_delta,
                        stage,
                        "max_duration_s",
                        "baseline",
                    )
                    candidate_case_stage_max = repeat_case_delta_ocr_stage_value(
                        case_delta,
                        stage,
                        "max_duration_s",
                        "candidate",
                    )
                    if baseline_case_stage_max is not None:
                        violation["baseline_ocr_engine_stage_max_duration_s"] = round(
                            baseline_case_stage_max,
                            6,
                        )
                    if candidate_case_stage_max is not None:
                        violation["candidate_ocr_engine_stage_max_duration_s"] = round(
                            candidate_case_stage_max,
                            6,
                        )
                    violations.append(violation)
    if max_repeat_ocr_engine_count_p95_regression is not None:
        max_repeat_ocr_count_p95 = round(
            float(max_repeat_ocr_engine_count_p95_regression),
            6,
        )
        budget["max_repeat_ocr_engine_count_p95_regression"] = max_repeat_ocr_count_p95
        violations.extend(
            repeat_ocr_count_regression_budget_violations(
                comparison,
                max_regression=max_repeat_ocr_engine_count_p95_regression,
                rounded_budget=max_repeat_ocr_count_p95,
                stat="p95_count",
                budget_key="max_repeat_ocr_engine_count_p95_regression",
                missing_kind="repeat_ocr_count_p95_delta_missing",
                aggregate_kind="repeat_ocr_count_p95_regression_exceeded",
                case_kind="repeat_ocr_case_count_p95_regression_exceeded",
            )
        )
    if max_repeat_ocr_engine_count_max_regression is not None:
        max_repeat_ocr_count_max = round(
            float(max_repeat_ocr_engine_count_max_regression),
            6,
        )
        budget["max_repeat_ocr_engine_count_max_regression"] = max_repeat_ocr_count_max
        violations.extend(
            repeat_ocr_count_regression_budget_violations(
                comparison,
                max_regression=max_repeat_ocr_engine_count_max_regression,
                rounded_budget=max_repeat_ocr_count_max,
                stat="max_count",
                budget_key="max_repeat_ocr_engine_count_max_regression",
                missing_kind="repeat_ocr_count_max_delta_missing",
                aggregate_kind="repeat_ocr_count_max_regression_exceeded",
                case_kind="repeat_ocr_case_count_max_regression_exceeded",
            )
        )
    if max_repeat_ocr_overlap_hidden_total_regression_s is not None:
        max_repeat_hidden_total = round(
            float(max_repeat_ocr_overlap_hidden_total_regression_s),
            6,
        )
        budget[
            "max_repeat_ocr_overlap_hidden_total_regression_s"
        ] = max_repeat_hidden_total
        repeat_delta = comparison.get("repeat_profile_delta")
        hidden_total_delta = (
            repeat_delta_metric_value(
                repeat_delta,
                ("ocr_overlap_hidden_s", "total_s"),
                "delta_s",
            )
            if isinstance(repeat_delta, dict)
            else None
        )
        if hidden_total_delta is None:
            violations.append(
                {
                    "kind": "repeat_ocr_overlap_hidden_total_delta_missing",
                    "max_repeat_ocr_overlap_hidden_total_regression_s": (
                        max_repeat_hidden_total
                    ),
                }
            ...
            violation = {
                ...
                "delta_s": round(hidden_total_delta, 6),
                "max_repeat_ocr_overlap_hidden_total_regression_s": (
                    max_repeat_hidden_total
                ),
            }
            baseline_hidden_total = repeat_delta_metric_value(
                repeat_delta,
                ("ocr_overlap_hidden_s", "total_s"),
                "baseline",
            )
            candidate_hidden_total = repeat_delta_metric_value(
                repeat_delta,
                ("ocr_overlap_hidden_s", "total_s"),
                "candidate",
            )
            if baseline_hidden_total is not None:
                violation["baseline_ocr_overlap_hidden_total_s"] = round(
                    baseline_hidden_total,
                    6,
                )
            if candidate_hidden_total is not None:
                violation["candidate_ocr_overlap_hidden_total_s"] = round(
                    candidate_hidden_total,
                    6,
                )
            violations.append(violation)
...
def repeat_ocr_count_regression_budget_violations(
    ...
    *,
    ...
    violations: list[dict[str, Any]] = []
    repeat_delta = comparison.get("repeat_profile_delta")
    aggregate_metric_seen = False
    if isinstance(repeat_delta, dict):
        count_metrics = repeat_delta.get("ocr_engine_count_metric")
        if isinstance(count_metrics, dict):
            for metric in OCR_ENGINE_COUNT_DISPLAY_KEYS:
                count_delta = repeat_delta_metric_value(
                    repeat_delta,
                    ("ocr_engine_count_metric", metric, stat),
                    "delta_count",
                )
                if count_delta is None:
                    continue
                aggregate_metric_seen = True
                if count_delta <= max_regression:
                    continue
                violation = {
                    "kind": aggregate_kind,
                    "metric": metric,
                ...
                if baseline_count is not None:
                    violation[f"baseline_{stat}"] = round(baseline_count, 6)
                if candidate_count is not None:
                    violation[f"candidate_{stat}"] = round(candidate_count, 6)
                violations.append(violation)
    if not aggregate_metric_seen:
    ...
    if isinstance(case_deltas, list):
        for case_delta in case_deltas:
            if not isinstance(case_delta, dict):
                continue
            count_metrics = case_delta.get("ocr_engine_count_metric")
            if not isinstance(count_metrics, dict):
                continue
            for metric in OCR_ENGINE_COUNT_DISPLAY_KEYS:
                count_delta = repeat_case_delta_ocr_count_value(
                    case_delta,
                    metric,
                    stat,
                    "delta_count",
                )
                if count_delta is None or count_delta <= max_regression:
                    continue
                violation = {
                    "kind": case_kind,
                    "slug": case_delta.get("slug"),
                ...
                baseline_count = repeat_case_delta_ocr_count_value(
                    case_delta,
                    metric,
                    stat,
                    "baseline",
                )
                candidate_count = repeat_case_delta_ocr_count_value(
                    case_delta,
                    metric,
                    stat,
                    "candidate",
...
def repeat_case_delta_ocr_count_value(
    case_delta: dict[str, Any],
    metric: str,
    stat: str,
    value_key: str,
    ...
    if not isinstance(count_metrics, dict):
        return None
    metric_delta = count_metrics.get(metric)
    if not isinstance(metric_delta, dict):
        return None
    stat_delta = metric_delta.get(stat)
    if not isinstance(stat_delta, dict):
...
def stress_row_latency_delta(
    ...
    ocr_count_deltas = stress_row_ocr_engine_count_deltas(baseline_row, candidate_row)
    if ocr_count_deltas:
        delta["ocr_engine_count_delta"] = ocr_count_deltas
        baseline_ocr_count_values = stress_row_ocr_engine_count_values(
            baseline_row,
            ocr_count_deltas,
        )
        candidate_ocr_count_values = stress_row_ocr_engine_count_values(
            candidate_row,
            ocr_count_deltas,
        )
        if baseline_ocr_count_values:
            delta["baseline_ocr_engine_count"] = baseline_ocr_count_values
        if candidate_ocr_count_values:
            delta["candidate_ocr_engine_count"] = candidate_ocr_count_values
...
def stress_row_ocr_engine_count_deltas(
    baseline_row: dict[str, Any],
    candidate_row: dict[str, Any],
) -> dict[str, float]:
    deltas: dict[str, float] = {}
...
def stress_row_ocr_engine_count_values(
    row: dict[str, Any],
    ocr_count_deltas: dict[str, float],
) -> dict[str, float]:
    profile = row.get("ocr_engine_profile")
    if not isinstance(profile, dict):
        return {}
    values: dict[str, float] = {}
    for key in OCR_ENGINE_COUNT_DISPLAY_KEYS:
        if key not in ocr_count_deltas:
            continue
        value = parse_nonnegative_count_metric(profile.get(key))
        if value is not None:
            values[key] = round(value, 6)
    return values
...
def build_latency_budget_summary(
    ...
    *,
    ...
    repeat_ocr_engine_max_violations = repeat_ocr_engine_max_budget_violations(
        repeat_profile,
        max_repeat_ocr_engine_max_duration_s or {},
    )
    ...
    summary = {
        "passed": (
            ...
            and not repeat_ocr_engine_max_violations
    ...
    if max_repeat_ocr_engine_max_duration_s:
        summary["max_repeat_ocr_engine_max_duration_s"] = {
            metric: round(float(seconds), 6)
            for metric, seconds in sorted(max_repeat_ocr_engine_max_duration_s.items())
        }
        summary["repeat_ocr_engine_max_violations"] = repeat_ocr_engine_max_violations
...
def repeat_ocr_engine_max_budget_violations(
    repeat_profile: dict[str, Any] | None,
    budgets: dict[str, float],
) -> list[dict[str, Any]]:
    if not budgets:
        ...
        return [
            {
                ...
                "metric": metric,
                "max_repeat_ocr_engine_max_duration_s": round(float(budget), 6),
            }
            for metric, budget in sorted(budgets.items())
        ]
    if repeat_profile_has_only_zero_ocr_engine_calls(repeat_profile):
    ...
    for metric, budget in sorted(budgets.items()):
        stats = stage_stats.get(metric)
        max_duration_s = stats.get("max_duration_s") if isinstance(stats, dict) else None
        parsed = parse_nonnegative_float(max_duration_s)
        if parsed is None:
            violations.append(
                {
                    "kind": "repeat_ocr_engine_max_missing",
...
def print_stress_table(report: dict[str, Any]) -> None:
    ...
    if isinstance(latency_budget, dict):
        ...
        if latency_budget.get("passed"):
            ...
            repeat_ocr_engine_max = latency_budget.get("repeat_ocr_engine_max_violations")
            repeat_ocr_engine_max_count = (
                len(repeat_ocr_engine_max) if isinstance(repeat_ocr_engine_max, list) else 0
            )
            ...
            print(
                ...
                f"repeat_ocr_engine_max={repeat_ocr_engine_max_count} "
            ...
            if isinstance(repeat_ocr_engine_max, list):
                for issue in repeat_ocr_engine_max[:6]:
                    if not isinstance(issue, dict):
                        continue
                    metric = issue.get("metric")
                    if issue.get("kind") == "repeat_ocr_engine_max_budget_exceeded":
                        print(
                            "   - "
                            f"{metric}: repeat OCR engine max {float(issue['max_duration_s']):.3f}s "
                            f"> budget {float(issue['max_repeat_ocr_engine_max_duration_s']):.3f}s"
                        )
                    elif issue.get("kind") == "repeat_ocr_engine_max_missing":
                        print(f"   - {metric}: repeat OCR engine max metric missing")
...
def baseline_regression_budget_text(regression_budget: dict[str, Any]) -> str:
    ...
    if max_repeat_stage is not None:
        limits.append(f"repeat_stage_p95<={max_repeat_stage:.3f}s")
    ...
    max_repeat_ocr_max = parse_nonnegative_float(
        regression_budget.get("max_repeat_ocr_engine_total_max_regression_s")
    )
    if max_repeat_ocr_max is not None:
        limits.append(f"repeat_ocr_max<={max_repeat_ocr_max:.3f}s")
    max_repeat_ocr_count_p95 = parse_nonnegative_float(
        regression_budget.get("max_repeat_ocr_engine_count_p95_regression")
    )
    if max_repeat_ocr_count_p95 is not None:
        limits.append(f"repeat_ocr_count_p95<={max_repeat_ocr_count_p95:.1f}")
    max_repeat_ocr_count_max = parse_nonnegative_float(
        regression_budget.get("max_repeat_ocr_engine_count_max_regression")
    )
    if max_repeat_ocr_count_max is not None:
        limits.append(f"repeat_ocr_count_max<={max_repeat_ocr_count_max:.1f}")
    max_repeat_hidden_total = parse_nonnegative_float(
        regression_budget.get("max_repeat_ocr_overlap_hidden_total_regression_s")
    )
    if max_repeat_hidden_total is not None:
        limits.append(f"repeat_ocr_hidden_total<={max_repeat_hidden_total:.3f}s")
...
def baseline_regression_budget_violation_kind_label(kind: str) -> str:
    labels = {
        ...
        "repeat_ocr_total_max_regression_exceeded": "repeat_ocr_max",
        "repeat_ocr_stage_max_regression_exceeded": "repeat_ocr_stage_max",
        "repeat_ocr_total_max_delta_missing": "repeat_ocr_max_missing",
        "repeat_ocr_case_total_max_regression_exceeded": "repeat_ocr_case_max",
        "repeat_ocr_case_stage_max_regression_exceeded": "repeat_ocr_case_stage_max",
        "repeat_ocr_count_p95_regression_exceeded": "repeat_ocr_count_p95",
        "repeat_ocr_count_p95_delta_missing": "repeat_ocr_count_p95_missing",
        "repeat_ocr_case_count_p95_regression_exceeded": "repeat_ocr_case_count_p95",
        "repeat_ocr_count_max_regression_exceeded": "repeat_ocr_count_max",
        "repeat_ocr_count_max_delta_missing": "repeat_ocr_count_max_missing",
        "repeat_ocr_case_count_max_regression_exceeded": "repeat_ocr_case_count_max",
        "repeat_ocr_overlap_hidden_total_regression_exceeded": "repeat_ocr_hidden_total",
        "repeat_ocr_overlap_hidden_total_delta_missing": "repeat_ocr_hidden_total_missing",
    }
...
def baseline_regression_budget_violation_text(violation: Any) -> str:
    ...
    if kind == "primary_ocr_count_regression_exceeded":
        slug = violation.get("slug")
        metric = violation.get("metric")
        delta = parse_signed_float(violation.get("delta_count"))
        budget = parse_nonnegative_float(violation.get("max_ocr_engine_count_regression"))
        if (
            not isinstance(slug, str)
            ...
            return ""
        metric_label = OCR_ENGINE_COUNT_DISPLAY_LABELS.get(metric, metric)
        return f"primary ocr count {slug} {metric_label} {delta:+.1f} > budget {budget:.1f}"
    if kind == "primary_ocr_count_delta_missing":
        slug = violation.get("slug")
        budget = parse_nonnegative_float(violation.get("max_ocr_engine_count_regression"))
        slug_text = f" {slug}" if isinstance(slug, str) and slug else ""
    ...
    if kind == "primary_stage_total_regression_exceeded":
        ...
        if not isinstance(stage, str) or delta is None or budget is None:
            return ""
        return f"primary stage {stage} {delta:+.3f}s > budget {budget:.3f}s"
    if kind == "primary_stage_total_delta_missing":
        budget = parse_nonnegative_float(violation.get("max_stage_total_regression_s"))
        budget_text = f" budget {budget:.3f}s" if budget is not None else ""
    ...
    if kind == "repeat_stage_p95_delta_missing":
        budget = parse_nonnegative_float(violation.get("max_repeat_stage_p95_regression_s"))
        budget_text = f" budget {budget:.3f}s" if budget is not None else ""
        return f"repeat stage p95 delta missing{budget_text}"
    ...
    if kind == "repeat_ocr_total_max_regression_exceeded":
        delta = parse_signed_float(violation.get("delta_s"))
        budget = parse_nonnegative_float(violation.get("max_repeat_ocr_engine_total_max_regression_s"))
        if delta is None or budget is None:
            return ""
        return f"repeat ocr max {delta:+.3f}s > budget {budget:.3f}s"
    if kind == "repeat_ocr_total_max_delta_missing":
        budget = parse_nonnegative_float(violation.get("max_repeat_ocr_engine_total_max_regression_s"))
        budget_text = f" budget {budget:.3f}s" if budget is not None else ""
        return f"repeat ocr max delta missing{budget_text}"
    if kind == "repeat_ocr_stage_max_regression_exceeded":
        stage = violation.get("stage")
        delta = parse_signed_float(violation.get("delta_s"))
        budget = parse_nonnegative_float(violation.get("max_repeat_ocr_engine_total_max_regression_s"))
        if not isinstance(stage, str) or delta is None or budget is None:
            return ""
        stage_label = baseline_repeat_ocr_stage_delta_label(stage)
        return f"repeat ocr stage {stage_label} max {delta:+.3f}s > budget {budget:.3f}s"
    if kind == "repeat_ocr_case_total_max_regression_exceeded":
        slug = violation.get("slug")
        delta = parse_signed_float(violation.get("delta_s"))
        budget = parse_nonnegative_float(
            violation.get("max_repeat_ocr_engine_total_max_regression_s")
        )
        if not isinstance(slug, str) or delta is None or budget is None:
            return ""
        return f"repeat ocr case max {slug} {delta:+.3f}s > budget {budget:.3f}s"
    if kind == "repeat_ocr_case_stage_max_regression_exceeded":
        slug = violation.get("slug")
        stage = violation.get("stage")
        delta = parse_signed_float(violation.get("delta_s"))
        budget = parse_nonnegative_float(
        ...
        stage_label = baseline_repeat_ocr_stage_delta_label(stage)
        return (
            f"repeat ocr case stage max {slug} {stage_label} "
            f"{delta:+.3f}s > budget {budget:.3f}s"
        )
    if kind in {
        "repeat_ocr_count_p95_regression_exceeded",
        "repeat_ocr_count_max_regression_exceeded",
    }:
        metric = violation.get("metric")
        delta = parse_signed_float(violation.get("delta_count"))
        budget_key = (
            "max_repeat_ocr_engine_count_p95_regression"
            if kind == "repeat_ocr_count_p95_regression_exceeded"
            else "max_repeat_ocr_engine_count_max_regression"
        )
        budget = parse_nonnegative_float(violation.get(budget_key))
        if not isinstance(metric, str) or delta is None or budget is None:
            return ""
        metric_label = OCR_ENGINE_COUNT_DISPLAY_LABELS.get(metric, metric)
        stat_label = "p95" if "p95" in kind else "max"
        return (
            f"repeat ocr count {metric_label} {stat_label} "
            f"{delta:+.1f} > budget {budget:.1f}"
        )
    if kind in {"repeat_ocr_count_p95_delta_missing", "repeat_ocr_count_max_delta_missing"}:
        budget_key = (
            "max_repeat_ocr_engine_count_p95_regression"
            if kind == "repeat_ocr_count_p95_delta_missing"
            else "max_repeat_ocr_engine_count_max_regression"
        )
        budget = parse_nonnegative_float(violation.get(budget_key))
        stat_label = "p95" if "p95" in kind else "max"
        budget_text = f" budget {budget:.1f}" if budget is not None else ""
        return f"repeat ocr count {stat_label} delta missing{budget_text}"
    if kind in {
        "repeat_ocr_case_count_p95_regression_exceeded",
        "repeat_ocr_case_count_max_regression_exceeded",
    }:
        slug = violation.get("slug")
        metric = violation.get("metric")
        delta = parse_signed_float(violation.get("delta_count"))
        budget_key = (
            "max_repeat_ocr_engine_count_p95_regression"
            if kind == "repeat_ocr_case_count_p95_regression_exceeded"
            else "max_repeat_ocr_engine_count_max_regression"
        )
        budget = parse_nonnegative_float(violation.get(budget_key))
        if (
        ...
        stat_label = "p95" if "p95" in kind else "max"
        return (
            f"repeat ocr case count {slug} {metric_label} {stat_label} "
            f"{delta:+.1f} > budget {budget:.1f}"
        )
    if kind == "repeat_ocr_overlap_hidden_total_regression_exceeded":
        delta = parse_signed_float(violation.get("delta_s"))
        budget = parse_nonnegative_float(
            violation.get("max_repeat_ocr_overlap_hidden_total_regression_s")
        )
        if delta is None or budget is None:
            return ""
        return f"repeat ocr hidden total {delta:+.3f}s > budget {budget:.3f}s"
    if kind == "repeat_ocr_overlap_hidden_total_delta_missing":
        budget = parse_nonnegative_float(
            violation.get("max_repeat_ocr_overlap_hidden_total_regression_s")
        )
        budget_text = f" budget {budget:.3f}s" if budget is not None else ""
        return f"repeat ocr hidden total delta missing{budget_text}"
    return ""
