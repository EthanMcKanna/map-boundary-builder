...
- Deployed the upload-parser hash/profile hardening as
  `dpl_4c8ekmjw3pK5vf9eK3ueuKH1zUQx`, aliased to
  `https://mapboundary.app`. Live health reported
  `pipeline-75b9631cf0a4b002` with RapidOCR 1.4.4 and onnxruntime 1.26.0
  (`out/prod-smoke-upload-hash-20260531/health.json`), and
  `/api/health?warm=ocr` stayed healthy with warm status `ok`
  (`out/prod-smoke-upload-hash-20260531/health-warm.json`). A fresh
  ...
  `[-86.8464007, 36.1088135, -86.6905828, 36.2428345]`, `catalog_slug: null`,
  `profile.pipeline_version: pipeline-75b9631cf0a4b002`, and
  `total_before_send_s=2.370332` (`cache_hit: miss`). The exact repeat hit the
  raw run-result cache with the same bbox/source/confidence and profile
...
- Deployed the cached catalog-miss event detail preservation as
  `dpl_B6qCZQ65oXzFUY48gAjnVMYT3N45`, aliased to `https://mapboundary.app`.
  Live health reported `pipeline-e108f15e143e4b7a`, and `/api/health?warm=ocr`
  stayed healthy with warm status `ok`
  (`out/prod-smoke-catalog-miss-event-details-20260531/health.json` and
  `out/prod-smoke-catalog-miss-event-details-20260531/health-warm.json`). A
  fresh generic catalog-probe miss returned `status: catalog_miss`,
  `profile.cache_hit: miss`, `profile.total_before_send_s=0.146782`,
  `profile.pipeline_version=pipeline-e108f15e143e4b7a`, and top-level
  `catalog_probe_miss.best_active_catalog_slug=dallas-tesla`. The exact repeat
  hit the raw run-result cache with `cached: true`, `profile.cache_hit: raw`,
  `profile.total_before_send_s=0.000445`, and the cached event details now
  preserved `best_active_catalog_slug=dallas-tesla`,
  `active_shape_iou_is_low=true`, and `style=gray-fill`
...
- Accepted a stricter filename-context fast path for OCR georeference. The
  Miami no-catalog drift was not fundamentally a stacked-label problem: a
  filename hint for `miami.png` was returning a merely credible narrow-city fit
...
- Accepted a narrower follow-up to the dense dark-teal header selector: after
  ...
  path improves without cross-style regressions. Production deployment proof:
  commit `fe3cdde` was built with Vercel CLI `54.3.0` using
  `PATH="$PWD/.venv/bin:$PATH" vercel build --prod` and deployed with
  `vercel deploy --prebuilt --prod --yes` as
  `dpl_9WJEpH6Z755EHzcY2cXixpqcr9xf`, aliased to
  `https://mapboundary.app`. Public `/api/health?warm=ocr`
  (`out/prod-footer-filter-health-20260602.json`) returned `ok: true`,
  pipeline `pipeline-d714ae987cf02cf8`, warm `status: ok`, warm total
  `2.563996s`, RapidOCR warm `2.125601s`, and `cv2 4.10.0`. A cache-busted
  original Grand Rapids no-catalog POST with `include_overlay=0` and
  `normalized_cache_lookup=0`
  (`out/prod-footer-filter-grand-rapids-20260602.json`) returned HTTP `201`,
  `cache_hit: miss`, city `Grand Rapids`, source
  `ocr-georeference:nominatim-label-fit`, `5` controls, `18` labels, the
  expected bbox, `build_boundary_s: 1.502052`, `total_before_send_s:
  1.509421`, and stage timings `extract=0.238591s`, `ocr=1.052420s`,
  `georeference=0.117329s`. A one-pixel cache-busted PNG variant
  (`out/prod-footer-filter-grand-rapids-pixelbust-20260602.json`) forced
  another run-cache miss on the warm instance and preserved the same
