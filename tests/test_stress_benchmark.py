...
def test_compare_stress_reports_records_signature_and_latency_delta() -> None:
    ...
    assert comparison["largest_total_improvements"][0]["candidate_stage_duration_s"] == {
        "ocr": 0.25
    }
    ...
    assert regression_budget["max_repeat_stage_p95_regression_s"] == 0.02
    ...
    assert regression_budget["violations"][3]["stage"] == "ocr"
    assert regression_budget["violations"][3]["delta_s"] == 0.15
    assert regression_budget["violations"][3]["baseline_stage_duration_s"] == 0.5
    assert regression_budget["violations"][3]["candidate_stage_duration_s"] == 0.65
    assert regression_budget["violations"][4]["delta_s"] == 0.03
    assert regression_budget["violations"][5]["slug"] == "houston"
    assert regression_budget["violations"][5]["delta_s"] == 0.06
    assert regression_budget["violations"][5]["baseline_p95_total_elapsed_s"] == 0.68
    assert regression_budget["violations"][5]["candidate_p95_total_elapsed_s"] == 0.74
    assert regression_budget["violations"][6]["stage"] == "ocr"
    assert regression_budget["violations"][6]["delta_s"] == 0.03
    assert regression_budget["violations"][6]["baseline_stage_p95_duration_s"] == 0.4
    assert regression_budget["violations"][6]["candidate_stage_p95_duration_s"] == 0.43
    assert regression_budget["violations"][7]["slug"] == "houston"
    assert regression_budget["violations"][7]["stage"] == "georeference"
    assert regression_budget["violations"][7]["delta_s"] == 0.05
    assert regression_budget["violations"][7]["baseline_stage_p95_duration_s"] == 0.18
    assert regression_budget["violations"][7]["candidate_stage_p95_duration_s"] == 0.23
    assert regression_budget["violations"][8]["delta_s"] == 0.02
    assert regression_budget["violations"][9]["slug"] == "houston"
    assert regression_budget["violations"][9]["delta_s"] == 0.05
    assert regression_budget["violations"][9]["baseline_ocr_engine_total_p95_duration_s"] == 0.31
...
def test_baseline_primary_ocr_count_budget_flags_growth_without_latency_regression() -> None:
    ...
    assert row_delta["ocr_engine_count_delta"] == {
        "raw_box_count": -1.0,
        "selected_box_count": 4.0,
        "label_count": 3.0,
    }
    assert row_delta["baseline_ocr_engine_count"] == {
        "raw_box_count": 42.0,
        "selected_box_count": 20.0,
        "label_count": 18.0,
    }
    assert row_delta["candidate_ocr_engine_count"] == {
        "raw_box_count": 41.0,
        "selected_box_count": 24.0,
        "label_count": 21.0,
    }
    assert comparison["regression_budget"]["passed"] is False
    assert comparison["regression_budget"]["violations"] == [
        {
            ...
            "delta_count": 4.0,
            "max_ocr_engine_count_regression": 2.0,
            "baseline_ocr_engine_count": 20.0,
            "candidate_ocr_engine_count": 24.0,
        },
        {
            "kind": "primary_ocr_count_regression_exceeded",
            "slug": "count-growth",
            ...
            "delta_count": 3.0,
            "max_ocr_engine_count_regression": 2.0,
            "baseline_ocr_engine_count": 18.0,
            "candidate_ocr_engine_count": 21.0,
        },
    ]
    assert (
        stress_module.baseline_regression_budget_text(comparison["regression_budget"])
        == "baseline regression budget: failed primary<=0.050s primary_ocr<=0.050s "
        "primary_ocr_count<=2.0 violations=2 by_kind=primary_ocr_count:2"
    )
    assert (
        stress_module.baseline_regression_budget_violation_text(
            comparison["regression_budget"]["violations"][0]
        )
        == "primary ocr count count-growth selected_box_count +4.0 > budget 2.0"
    )
    assert (
        stress_module.baseline_regression_budget_violation_text(
            comparison["regression_budget"]["violations"][1]
        )
        == "primary ocr count count-growth label_count +3.0 > budget 2.0"
    )
...
def test_baseline_repeat_ocr_hidden_total_budget_skips_mismatched_sample_counts() -> None:
    baseline = {
        "rows": [
            {
                "slug": "hidden-repeat",
    ...
    repeat_hidden = comparison["repeat_profile_delta"]["ocr_overlap_hidden_s"]
    assert repeat_hidden["total_s"]["delta_s"] == 0.061
    assert comparison["repeat_profile_delta"]["sample_counts"]["analyzed_samples"] == {
        "baseline": 2.0,
        "candidate": 3.0,
        "delta": 1.0,
    }
    ...
    assert comparison["regression_budget"][
        ...
        "baseline_analyzed_samples": 2.0,
        "candidate_analyzed_samples": 3.0,
        "delta_samples": 1.0,
    }
    assert (
        stress_module.baseline_regression_budget_text(comparison["regression_budget"])
        == "baseline regression budget: passed repeat_p95<=0.050s "
        "repeat_ocr_p95<=0.050s repeat_ocr_hidden_total<=0.050s "
        "skipped_repeat_ocr_hidden_total_sample_mismatch=2->3"
    )
...
def test_baseline_repeat_ocr_max_budget_flags_outlier_without_p95_regression() -> None:
    baseline = {
        "rows": [
            {
                "slug": "case-only",
        ...
        "repeat_profile": {
            "summary": {
                "ocr_engine_stage_duration_s": {
                    "input_s": {"p95_duration_s": 0.01, "max_duration_s": 0.02},
                    "det_elapsed_s": {"p95_duration_s": 0.2, "max_duration_s": 0.24},
                    "rec_elapsed_s": {"p95_duration_s": 0.2, "max_duration_s": 0.25},
                    "total_s": {"p95_duration_s": 0.42, "max_duration_s": 0.5},
                },
            },
            "cases": {
                "case-only": {
    ...
    candidate = {
        ...
        "repeat_profile": {
            "summary": {
                "ocr_engine_stage_duration_s": {
                    "input_s": {"p95_duration_s": 0.01, "max_duration_s": 0.02},
                    "det_elapsed_s": {"p95_duration_s": 0.19, "max_duration_s": 0.23},
                    "rec_elapsed_s": {"p95_duration_s": 0.19, "max_duration_s": 0.34},
                    "total_s": {"p95_duration_s": 0.41, "max_duration_s": 0.58},
                },
            },
            "cases": {
                "case-only": {
    ...
    comparison = stress_module.compare_stress_reports(
        ...
        candidate,
        max_repeat_ocr_engine_total_p95_regression_s=0.05,
        max_repeat_ocr_engine_total_max_regression_s=0.05,
    )
    ...
    assert comparison["regression_budget"]["violations"] == [
        {
            ...
            "delta_s": 0.08,
            "max_repeat_ocr_engine_total_max_regression_s": 0.05,
            "baseline_ocr_engine_total_max_duration_s": 0.5,
            "candidate_ocr_engine_total_max_duration_s": 0.58,
        },
        {
            ...
            "delta_s": 0.09,
            "max_repeat_ocr_engine_total_max_regression_s": 0.05,
            "baseline_ocr_engine_stage_max_duration_s": 0.25,
            "candidate_ocr_engine_stage_max_duration_s": 0.34,
        },
        {
    ...
    assert (
        stress_module.baseline_regression_budget_text(comparison["regression_budget"])
        == "baseline regression budget: failed repeat_ocr_p95<=0.050s "
        "repeat_ocr_max<=0.050s violations=4 "
        "by_kind=repeat_ocr_max:1,repeat_ocr_stage_max:1,"
        "repeat_ocr_case_max:1,repeat_ocr_case_stage_max:1"
    )
    assert (
        stress_module.baseline_regression_budget_violation_text(
            comparison["regression_budget"]["violations"][0]
        )
        == "repeat ocr max +0.080s > budget 0.050s"
    )
    assert (
        stress_module.baseline_regression_budget_violation_text(
            comparison["regression_budget"]["violations"][1]
        )
        == "repeat ocr stage rec max +0.090s > budget 0.050s"
    )
    assert (
        stress_module.baseline_regression_budget_violation_text(
            comparison["regression_budget"]["violations"][2]
        )
        == "repeat ocr case max case-only +0.080s > budget 0.050s"
    )
    assert (
        stress_module.baseline_regression_budget_violation_text(
            comparison["regression_budget"]["violations"][3]
        )
        == "repeat ocr case stage max case-only rec +0.090s > budget 0.050s"
    )


def test_baseline_repeat_ocr_count_budgets_flag_growth_without_latency_regression() -> None:
    baseline = {
        "rows": [
            {
                "slug": "case-only",
        ...
        "repeat_profile": {
            "summary": {
                ...
                "ocr_engine_count_metric": {
                    "selected_box_count": {"p95_count": 20.0, "max_count": 21.0},
                    "raw_box_count": {"p95_count": 44.0, "max_count": 45.0},
                },
            },
            "cases": {
                "case-only": {
                    ...
                    "ocr_engine_count_metric": {
                        "selected_box_count": {"p95_count": 10.0, "max_count": 11.0},
                        "raw_box_count": {"p95_count": 22.0, "max_count": 23.0},
                    },
                }
            },
        },
    ...
    candidate = {
        ...
        "repeat_profile": {
            "summary": {
                "ocr_engine_stage_duration_s": {
                    "total_s": {"p95_duration_s": 0.41, "max_duration_s": 0.49},
                },
                "ocr_engine_count_metric": {
                    "selected_box_count": {"p95_count": 23.0, "max_count": 25.0},
                    "raw_box_count": {"p95_count": 43.0, "max_count": 44.0},
                },
            },
            "cases": {
                "case-only": {
                    ...
                    "ocr_engine_count_metric": {
                        "selected_box_count": {"p95_count": 13.0, "max_count": 14.0},
                        "raw_box_count": {"p95_count": 21.0, "max_count": 22.0},
                    },
                }
            },
        },
    ...
    assert (
        comparison["repeat_profile_delta"]["ocr_engine_count_metric"]["selected_box_count"][
            "p95_count"
        ]["delta_count"]
        == 3.0
    )
    assert (
        comparison["repeat_profile_case_deltas"][0]["ocr_engine_count_metric"][
            "selected_box_count"
        ]["max_count"]["delta_count"]
        == 3.0
    )
    assert comparison["regression_budget"]["passed"] is False
    assert comparison["regression_budget"]["violations"] == [
        {
            "kind": "repeat_ocr_count_p95_regression_exceeded",
            "metric": "selected_box_count",
            "delta_count": 3.0,
            "max_repeat_ocr_engine_count_p95_regression": 2.0,
            "baseline_p95_count": 20.0,
            "candidate_p95_count": 23.0,
        },
        {
            "kind": "repeat_ocr_case_count_p95_regression_exceeded",
            "slug": "case-only",
            "metric": "selected_box_count",
            "delta_count": 3.0,
            "max_repeat_ocr_engine_count_p95_regression": 2.0,
            "baseline_p95_count": 10.0,
            "candidate_p95_count": 13.0,
        },
        {
            "kind": "repeat_ocr_count_max_regression_exceeded",
            "metric": "selected_box_count",
            "delta_count": 4.0,
            "max_repeat_ocr_engine_count_max_regression": 2.0,
            "baseline_max_count": 21.0,
            "candidate_max_count": 25.0,
        },
        {
            "kind": "repeat_ocr_case_count_max_regression_exceeded",
            "slug": "case-only",
            "metric": "selected_box_count",
            "delta_count": 3.0,
            "max_repeat_ocr_engine_count_max_regression": 2.0,
            "baseline_max_count": 11.0,
            "candidate_max_count": 14.0,
        },
    ]
    assert (
        stress_module.baseline_regression_budget_text(comparison["regression_budget"])
        == "baseline regression budget: failed repeat_ocr_p95<=0.050s "
        "repeat_ocr_count_p95<=2.0 repeat_ocr_count_max<=2.0 violations=4 "
        "by_kind=repeat_ocr_count_p95:1,repeat_ocr_case_count_p95:1,"
        "repeat_ocr_count_max:1,repeat_ocr_case_count_max:1"
    )
    assert (
        stress_module.baseline_regression_budget_violation_text(
            comparison["regression_budget"]["violations"][0]
        )
        == "repeat ocr count selected_box_count p95 +3.0 > budget 2.0"
    )
    assert (
        stress_module.baseline_regression_budget_violation_text(
            comparison["regression_budget"]["violations"][1]
        )
        == "repeat ocr case count case-only selected_box_count p95 +3.0 > budget 2.0"
    )
    assert (
        stress_module.baseline_regression_budget_violation_text(
            comparison["regression_budget"]["violations"][2]
        )
        == "repeat ocr count selected_box_count max +4.0 > budget 2.0"
    )
    assert (
        stress_module.baseline_regression_budget_violation_text(
            comparison["regression_budget"]["violations"][3]
        )
        == "repeat ocr case count case-only selected_box_count max +3.0 > budget 2.0"
    )


def test_baseline_repeat_max_budget_flags_outlier_without_p95_regression() -> None:
    baseline = {
        "rows": [
            {
                "slug": "case-only",
    ...
    comparison = stress_module.compare_stress_reports(
        ...
        candidate,
        max_repeat_p95_regression_s=0.05,
        max_repeat_max_regression_s=0.05,
    )

    assert comparison["repeat_profile_delta"]["duration_s"]["p95_total_elapsed_s"][
        "delta_s"
    ] == -0.01
    assert comparison["repeat_profile_delta"]["duration_s"]["max_total_elapsed_s"][
        "delta_s"
    ] == 0.09
    assert comparison["repeat_profile_case_deltas"][0]["duration_s"][
        "p95_total_elapsed_s"
    ]["delta_s"] == -0.01
    assert comparison["repeat_profile_case_deltas"][0]["duration_s"][
        "max_total_elapsed_s"
    ...
    assert comparison["regression_budget"]["violations"] == [
        {
            ...
            "delta_s": 0.09,
            "max_repeat_max_regression_s": 0.05,
            "baseline_max_total_elapsed_s": 0.55,
            "candidate_max_total_elapsed_s": 0.64,
        },
        {
    ...
    assert (
        ...
        == "repeat max +0.090s > budget 0.050s"
    )
    assert (
        stress_module.baseline_regression_budget_violation_text(
            comparison["regression_budget"]["violations"][1]
        )
        == "repeat case max case-only +0.090s > budget 0.050s"
    )
...
def test_compare_stress_reports_scopes_missing_candidate_for_focused_reports() -> None:
    ...
    assert comparison["regression_budget"]["violations"] == [
        ...
        },
        {
            ...
            "delta_s": 0.05,
            "max_stage_total_regression_s": 0.04,
            "baseline_stage_duration_s": 0.2,
            "candidate_stage_duration_s": 0.25,
        }
    ]
    repeat_delta = comparison["repeat_profile_delta"]
...
def test_baseline_regression_budget_violation_samples_include_each_kind() -> None:
    ...
    violations.extend(
        [
            ...
            },
            {
                "kind": "repeat_profile_max_regression_exceeded",
                "delta_s": 0.13,
                "max_repeat_max_regression_s": 0.03,
            },
            {
                "kind": "repeat_profile_case_max_regression_exceeded",
                "slug": "case-max-slow",
    ...
    samples = stress_module.baseline_regression_budget_violation_samples(violations, limit=15)
    ...
    assert sample_kinds == [
        ...
        "repeat_profile_p95_regression_exceeded",
        "repeat_profile_case_p95_regression_exceeded",
        "repeat_profile_max_regression_exceeded",
        "repeat_profile_case_max_regression_exceeded",
        "repeat_stage_p95_regression_exceeded",
        "repeat_stage_case_p95_regression_exceeded",
        "repeat_ocr_total_p95_regression_exceeded",
    ...
    assert stress_module.baseline_regression_budget_violation_count_text(violations) == (
        ...
        "repeat_p95:1,repeat_case_p95:1,repeat_max:1,repeat_case_max:1,"
        "repeat_stage_p95:1,repeat_stage_case_p95:1,"
    ...
    assert (
        ...
        == "repeat max +0.130s > budget 0.030s"
    )
    assert (
        stress_module.baseline_regression_budget_violation_text(violations[13])
        == "repeat case max case-max-slow +0.110s > budget 0.030s"
    )
    assert (
    ...
    assert (
        stress_module.baseline_regression_budget_violation_text(violations[18])
        == "repeat ocr case p95 ocr-case-slow +0.070s > budget 0.010s"
    )
    assert (
        stress_module.baseline_regression_budget_violation_text(violations[19])
...
def test_real_screenshot_manifest_preserves_ocr_contract_coverage() -> None:
    ...
    budget = stress_module.build_manifest_contract_budget_summary(
        ...
        min_ocr_count_contract_rows=45,
        max_positive_ocr_call_only_rows=0,
        fail_on_invalid_ocr_count_contracts=True,
    )
    ...
    assert contracts["ocr_zero_call_contract_rows"] == 28
...
def test_main_applies_real_screenshot_hard_gate_preset(tmp_path, monkeypatch) -> None:
    def fake_run_stress_benchmark(manifest_path, out_dir, **kwargs):
        ...
        assert kwargs["min_ocr_count_contract_rows"] == 45
        ...
        return {
            ...
            "manifest_contracts": {
                ...
                "ocr_positive_call_contract_rows": 45,
                "ocr_zero_call_contract_rows": 5,
                "ocr_count_contract_rows": 45,
                "ocr_positive_call_rows_without_count_contract": [],
            },
            "manifest_contract_budget": {
                "passed": True,
