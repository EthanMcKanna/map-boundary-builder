...
def test_route_ui_fast_ocr_cap_only_targets_tall_phone_route_screens(monkeypatch) -> None:
    ...
    monkeypatch.setattr(runner, "GRAY_FILL_ROUTE_UI_OCR_MAX_DIMENSION", 900)

    assert runner.route_ui_fast_ocr_max_dimension_for_style("light-fill", width=1206, height=2622) == 1000
    assert runner.route_ui_fast_ocr_max_dimension_for_style("gray-fill", width=1320, height=2714) == 900
    assert runner.route_ui_fast_ocr_max_dimension_for_style("light-fill", width=680, height=551) is None
    assert runner.route_ui_fast_ocr_max_dimension_for_style("gray-fill", width=680, height=551) is None
    ...
    assert runner.route_ui_fast_ocr_max_dimension_for_style("gray-fill", width=1500, height=2622) is None
    assert runner.route_ui_fast_ocr_max_dimension_for_style("light-fill", width=1206, height=1600) is None
    assert runner.route_ui_fast_ocr_max_dimension_for_style("gray-fill", width=1320, height=1800) is None
...
def test_fast_text_ocr_filter_only_applies_to_safe_styles(monkeypatch) -> None:
    ...
    monkeypatch.setattr(runner, "BRIGHT_BLUE_FAST_TEXT_OCR_MIN_AREA", 2300.0)
    monkeypatch.setattr(runner, "SVG_BRIGHT_BLUE_FAST_TEXT_OCR_MIN_AREA", 300.0)

    assert runner.fast_text_ocr_min_area_for_style("bright-blue") == 2300.0
...
def test_fast_text_ocr_can_be_disabled(monkeypatch) -> None:
    ...
    monkeypatch.setattr(runner, "BRIGHT_BLUE_FAST_TEXT_OCR_MIN_AREA", 0.0)
    ...
    assert runner.fast_text_ocr_min_area_for_style("gray-fill") is None
    ...
    assert ocr_kwargs == [
        {
            ...
            "rapidocr_min_text_area": runner.BRIGHT_BLUE_FAST_TEXT_OCR_MIN_AREA,
...
def test_post_georeference_catalog_completion_ranks_verified_screenshot_alias(monkeypatch) -> None:
    extraction = ExtractionResult(
        mask=np.ones((20, 20), dtype=bool),
        style="dark-teal",
        pixel_geometry=Polygon([(0, 0), (20, 0), (20, 20), (0, 20)]),
    ...
    bay_area = SimpleNamespace(
        ...
        slug="bay-area-zoox",
        catalog_source="current-external-service-area-reference",
        geometry=Polygon([(0, 0), (16, 0), (16, 16), (0, 16)]),
        mercator_geometry=Polygon([(0, 0), (16, 0), (16, 16), (0, 16)]),
        max_confidence=0.99,
        use_exact_geometry=True,
    )
    san_francisco = SimpleNamespace(
    ...
    georef_output = Polygon([(-0.2, -0.2), (10.2, -0.2), (10.2, 10.2), (-0.2, 10.2)])

    monkeypatch.setattr(runner, "load_catalog_entries", lambda: [bay_area, san_francisco])
    monkeypatch.setattr(runner, "lonlat_to_mercator", lambda lon, lat: (lon, lat))
    ...
    assert match is not None
    assert match.entry.slug == "san-francisco-zoox"
    assert match.iou == pytest.approx(100 / 108.16)
    assert match.iou - (108.16 / (256 + 108.16 - 108.16)) > runner.POST_GEOREF_CATALOG_COMPLETION_MIN_IOU_MARGIN
    assert match.confidence == pytest.approx(0.946)


def test_post_georeference_catalog_completion_keeps_broader_current_alias(monkeypatch) -> None:
    extraction = ExtractionResult(
        mask=np.ones((20, 20), dtype=bool),
        style="dark-teal",
        pixel_geometry=Polygon([(0, 0), (20, 0), (20, 20), (0, 20)]),
    ...
    georef_output = Polygon([(0, 0), (16, 0), (16, 16), (0, 16)])
    ...
    assert match is not None
    assert match.entry.slug == "bay-area-zoox"
    assert match.iou == pytest.approx(1.0)
...
def test_post_georeference_catalog_completion_rejects_ambiguous_alias_margin(monkeypatch) -> None:
    ...
    second = SimpleNamespace(
        ...
        slug="bay-area-zoox",
        catalog_source="current-external-service-area-reference",
        geometry=Polygon([(0.3, 0.3), (10.3, 0.3), (10.3, 10.3), (0.3, 10.3)]),
        mercator_geometry=Polygon([(0.3, 0.3), (10.3, 0.3), (10.3, 10.3), (0.3, 10.3)]),
        max_confidence=0.99,
        use_exact_geometry=True,
    )
    georef_output = Polygon([(0.15, 0.15), (10.15, 0.15), (10.15, 10.15), (0.15, 10.15)])

    monkeypatch.setattr(runner, "load_catalog_entries", lambda: [first, second])
    monkeypatch.setattr(runner, "lonlat_to_mercator", lambda lon, lat: (lon, lat))
...
def test_post_georeference_catalog_completion_rejects_lower_ranked_passing_alias(monkeypatch) -> None:
    extraction = ExtractionResult(
        mask=np.ones((20, 20), dtype=bool),
        style="dark-teal",
        pixel_geometry=Polygon([(0, 0), (20, 0), (20, 20), (0, 20)]),
    ...
    overgrown_output = Polygon([(-1, -1), (11.5, -1), (11.5, 11.5), (-1, 11.5)])
